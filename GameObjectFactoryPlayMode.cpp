...
void GameObjectFactoryPlayMode::buildGameObject(GameObjectBlueprint& bp, std::vector<GameObject>& gameObjects) {
    ...
    for(const auto& it : bp.getComponentList()) {
        if (it == "Transform")
            gameObject.addComponent(make_shared<TransformComponent>(bp.getWidth(), bp.getHeight(),
        ...
        else if (it == "RandomObstacle Update") {
            gameObject.addComponent(make_shared<RandomObstacleUpdateComponent>());
        }
    }
